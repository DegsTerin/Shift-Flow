...
- Activity mutations, audit records and activity-history evidence are not yet
  one atomic transaction. List `items` and `total` are likewise not yet read
  from one database snapshot.
- Activity task-board and dashboard reads still provision defaults. Task-board
  reorder and move operations remain vulnerable to invalid subsets, duplicate
  positions and partial writes, and attachment identifiers are not yet fully
  validated against the active tenant and record.
- Generic activity and task updates still permit lifecycle or movement fields
  to bypass specialised commands. The dashboard completed-duration sample is
  bounded but not yet deterministically selected.
- The web client does not yet consume server pagination end to end, limit its
  ...
  sending the request. The larger request-race, capability, Kanban and
  accessibility findings remain assigned to step 7.
