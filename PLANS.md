...
- Central Activity commands now compose Activity, AuditLog and ActivityHistory
  ...
  remain unverified without a disposable PostgreSQL target. List `items` and
  `total` are still not read from one database snapshot.
...
- Generic task updates still permit lifecycle or movement fields to bypass
  specialised commands. The dashboard completed-duration sample is bounded but
  not yet deterministically selected.
