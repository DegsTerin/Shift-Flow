...
const statuses = ["PENDING", "IN_PROGRESS", "WAITING_CUSTOMER", "WAITING_THIRD_PARTY", "MONITORING", "DONE"];
const priorities = ["LOW", "MEDIUM", "HIGH", "CRITICAL"];
const systems = ["Core Banking", "Payments", "API Gateway", "Identity", "Observability"];
const services = ["Incident response", "Client contact", "Settlement", "Access request", "Availability analysis"];
...
async function main() {
  ...
  for (let index = 1; index <= volume; index += 1) {
    ...
    activities.push({
      ...
      requested: `Homologation request ${index} for operational validation.`,
      performed: "Fixture triage completed by homologation seed.",
      inProgressDetail: status === "IN_PROGRESS" ? "Homologation execution is in progress." : "No active execution at this stage.",
      pendingDetail: status === "DONE" ? "No pending homologation action." : "Pending follow-up from the next operational update.",
      finalizationDetail: status === "DONE" ? "Homologation item finalised." : "Finalisation is not available yet.",
